...
int main(void) {
    ...
    while (!input.quit_requested) {
        ...
        if (color_input_mode) {
            if (input.keys_pressed[PLATFORM_KEY_BACKSPACE] && color_input_len > 0) {
                color_input_buf[--color_input_len] = '\0';
                printf("\r\x1b[Kcolor input: #%s", color_input_buf);
                fflush(stdout);
            }
            if (input.keys_pressed[PLATFORM_KEY_ENTER]) {
                u32 parsed;
            ...
            if (input.keys_pressed[PLATFORM_KEY_ESCAPE]) {
            ...
            if (input.keys_pressed[PLATFORM_KEY_Q]) input.quit_requested = true;
            if (input.keys_pressed[PLATFORM_KEY_M]) {
            ...
            if (input.keys_pressed[PLATFORM_KEY_1]) pattern = PATTERN_SOLID;
        ...
        /* Mouse output is suppressed while typing a color — otherwise it
           would garble the in-place "color input: #..." line. */
        if (!color_input_mode) {
